...
request = BroadcastRequest("localhost", 5000)

while True:
	responses = []
	def test(request_flow, response):
		responses.append(response["body"]["result"])

	q = raw_input("enter your search term: ")
	
	request.send(100, 5000, "application/json", "utf-8", { "q" : q }, test)
